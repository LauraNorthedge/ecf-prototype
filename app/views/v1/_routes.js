...
router.post('/one-login', function(req, res) {
	...
	if(req.session.data.version == '1.1')
		res.redirect("/v1/welcome");
	else 
		res.redirect("/v1/one-login/enter-password");
...
router.get(/select-user-type/, function(req, res) {
	...
	res.render('/v1/account-creation-org-admin/select-user-type', { user: user});
...
router.post(/select-user-type/, function(req, res) {
	...
	if (req.session.data.usertype == undefined) {
		res.redirect('/v1/account-creation-org-admin/select-user-type')
	}
	...
	if (req.session.data.id) {
		...
		res.redirect(`/v1/account-creation-org-admin/update-user?id=${req.session.data.id}&success=true`);

	} else {

		res.redirect(`/v1/account-creation-org-admin/add-user`);
...
router.get('/account-creation-org-admin/add-user', function(req, res) {
	...
	res.render('/v1/account-creation-org-admin/add-user', {user: user});
...
router.post('/account-creation-org-admin/add-user', function(req, res) {
	...
	if (req.session.data.id) {
		...
		res.redirect(`/v1/account-creation-org-admin/update-user?id=${req.session.data.id}`);
		...
		res.redirect(`/v1/org-admin?id=${req.body.id}`);
...
router.get('/account-creation-org-admin/update-user', function(req, res) {
	...
	res.render('/v1/account-creation-org-admin/update-user', {user: user});
...
router.get(/unlink-user/, function(req, res) {
	...
	res.redirect(`/v1/org-admin?id=${req.query.id}`);
...
router.get(/deactivate-account/, function(req, res) {
	...
	if (user.type == "coordinator" || user.type == "assessorCoordinator") {
		...
		if (countCoordinators == 1) {
			...
			res.redirect('/v1/account-creation-org-admin/cannot-deactivate');
